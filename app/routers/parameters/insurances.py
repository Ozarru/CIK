...
@router.get('/', response_model=List[fin_schemas.Insurance])
def get_insurances(db: Session = Depends(get_db), limit: int = 0, offset: int = 0, search: Optional[str] = ""):
...
@router.get('/{id}', response_model=fin_schemas.Insurance.update_forward_refs())
def get_insurance(id: int, db: Session = Depends(get_db),):
...
def create_insurance(insurance: fin_schemas.Insurance, db: Session = Depends(get_db), ):
